...
//	    CloudwatchLogStream:            cloudwatchLogStream,
		...
		Route53ResolverRule:                        route53ResolverRules,
...
func dxGatewayAssociations(ctx context.Context, a *aws, resourceType string, filters *filter.Filter) ([]provider.Resource, error) {
	...
	for _, gatewayID := range associatedGatewayID {
		...
		r, err := initializeResource(a, fmt.Sprintf("%s/%s", gatewayID, *i.AssociatedGateway.Id), resourceType)
		if err != nil {
			return nil, err
		}
		resources = append(resources, r)
...
func route53ResolverRules(ctx context.Context, a *aws, resourceType string, filters *filter.Filter) ([]provider.Resource, error) {
	resolverRule, err := a.awsr.GetResolverRules(ctx, nil)
	if err != nil {
		return nil, err
	}
